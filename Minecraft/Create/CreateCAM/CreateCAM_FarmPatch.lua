...
B
C - Замощение прямоугольника построчно с возвратом (пачт для ферм)
...
-- Стороны
local in_zero_side = "back" -- Вход датчика обнуления
local out_y_side = "top" -- Включение оси Y
local out_z_side = "left" -- Включение оси Z
local out_activetor_side = "bottom" -- Вкл/выкл активатора
...
function isOnZero()
	return not rs.getInput(in_zero_side)
end
...
function update_redstone(axis)
	rs.setOutput(out_activetor_side, not isEnabled())
...
function move_zero()
	for i=1,5 do
		...
		move_rel("Y", (-max_pos["Y"]/axis_mult["Y"])*2, true)
		move_rel("X", (-max_pos["X"]/axis_mult["X"])*2, true)
		move_rel("Z", (0-max_pos["Z"])*2, true)
	end
...
-- Замощение прямоугольника построчно с возвратом (патч для ферм)
function move_farm(x,y,z)
	x = (tonumber(x) or max_pos["X"])/axis_mult["C"]
	y = (tonumber(y) or max_pos["Y"])/axis_mult["Y"]
	z = tonumber(z) or 0
	...
	while i <= ax do
		...
		move_rel("Y", -y)
		if i ~= ax then
			move_rel("X", sx*axis_mult["C"])
		end
		i = i + 1
	end
	move_rel("X", -x*axis_mult["C"])
...
function eval(data)
	...
	elseif data:sub(1,1) == "C" then
		move_farm(split_size(data:sub(2)))
