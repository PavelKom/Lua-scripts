...
function this_library:CraftTask(name, count, fingerprint, nbt, batch, isFluid, triggers, isOR)
	...
	local ret = {
		...
		triggers=triggers or this_library:Triggers(name, count, nbt, _, isOR)
	}
	...
	ret.craft = function(interface, isOR, force_batch, callback)
		...
		if callback and type(callback) == 'function' then
			callback({result=result, nbt=ret.nbt, name=ret.name, fingerprint=ret.fingerprint, amount=amount})
		end
	...
	ret.craftFluid = function(interface, isOR, force_batch)
		if not ret.test(interface, isOR) then return false, -1 end
		if interface.isItemCrafting({name=name, nbt=ret.nbt, fingerprint=fingerprint}) then return false, -2 end
		batch = ret.batch or force_batch
		local result = interface.craftFluid({name=name, nbt=ret.nbt, fingerprint=fingerprint, count=batch})
...
function this_library:Trigger(name, fingerprint, count, nbt, operator)
	...
	ret.test = function(interface)
		if interface == nil then return false end
		local item = interface.getItem({name=ret.name, nbt=ret.nbt, fingerprint=ret.fingerprint})
